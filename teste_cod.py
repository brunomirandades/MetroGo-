...
class Application():
	...
	def __init__(self, master=None):
		...
		"""Container Geral"""
		self.containerGeral = Frame(master, bg=self.bgColor, height=600, width=800)
		self.containerGeral.pack()
		

		"""Container banner"""
		self.containerBanner = Frame(self.containerGeral, bg=self.bgColor, height=75, width=800)
		...
		"""Container do título"""
		self.containerTitulo = Frame(self.containerGeral, height=30, width=600, bg=self.bgColor)
		...
		self.containerConsultaInclusao = Frame(self.containerGeral, height=528, width=800, bg=self.bgColor)
		self.containerConsultaInclusao.pack(pady=(10,20))


		"""Container de consulta"""
		self.containerConsulta = Frame(self.containerConsultaInclusao, height=525, width=400, bg=self.bgColor)
		self.containerConsulta.pack(side=LEFT, padx=(0,30), pady=(18,0))
		# 
		# Container de consulta - Campos
		self.containerConsultaCampos = Frame(self.containerConsulta, bg=self.bgColor)
		...
		self.listboxNomeLinhaConsulta = Listbox(self.containerConsultaCampos, width=26, height=5, font=self.fonteEntry)
		...
		self.containerConsultaBotoes = Frame(self.containerConsulta, bg=self.bgColor)
		...
		self.containerIncluir = Frame(self.containerConsultaInclusao, height=525, width=400, bg=self.bgColor)
		self.containerIncluir.pack(side=LEFT, padx=(50,0))
		# 
		#  Container de inclusão - Campos
		self.containerIncluirCampos = Frame(self.containerIncluir, bg=self.bgColor)
		...
		self.containerIncluirRadioBtn = Frame(self.containerIncluir, bg=self.bgColor)
		...
		self.containerIncluirBotao = Frame(self.containerIncluir, bg=self.bgColor)
		self.containerIncluirBotao.pack(pady=15)
		...
		"""Container Rodapé"""
		self.containerRodape = Frame(self.containerGeral, bg=self.bgColor)
		self.containerRodape.pack(padx=(660,0), pady=(28,2))
	...
	def construirJanelaEdicao(self):
		...
		"""Elementos da tela"""
		containerGeral = Frame(janelaEdicao, bg=self.bgColor)
		containerGeral.pack()
		# 
		# titulo
		containerTitulo = Frame(containerGeral, bg=self.bgColor)
		containerTitulo.pack(pady=(10,0))
		labelTitulo = Label(containerTitulo, text="EDIÇÃO DE ESTAÇÃO", font=self.fonteTitulo, bg=self.bgColor)
		labelTitulo.pack()
		# 
		# campos
		containerCampos = Frame(containerGeral, bg=self.bgColor)
		...
		entryTransferencia = ttk.Combobox(containerCampos, font=self.fonteEntry, width=22, state="disabled")
		...
		# 
		# nome linha
		labelNomeAdmin = Label(containerCampos, text="LINHA (ÚNICA)", font=self.fonteLabels, bg=self.bgColor)
		labelNomeAdmin.pack(anchor=W, pady=(5,0))
		entryNomeAdmin = ttk.Combobox(containerCampos, font=self.fonteEntry, width=22, state="disabled")
		entryNomeAdmin["values"] = self.optionsNomeLinha
		# entryNomeAdmin.bind("<<ComboboxSelected>>", funcao)
		entryNomeAdmin.pack(anchor=W)
		# 
		# botoes
		containerBotoes = Frame(containerGeral, bg=self.bgColor)
		containerBotoes.pack(pady=10)
	...
	def construirJanelaMensagem(self, mensagem=None):
		...
		"""Elementos da tela"""
		containerGeral = Frame(janelaMensagem, bg=self.bgColor)
		containerGeral.pack()
		# 
		# mensagem
		containerMensagem = Frame(containerGeral, bg=self.bgColor)
	...
	def construirJanelaConfirmacao(self, mensagem=None):
		...
		"""Elementos da tela"""
		containerGeral = Frame(janelaConfirmacao, bg=self.bgColor)
		containerGeral.pack()
		# 
		# mensagem
		containerMensagem = Frame(containerGeral, bg=self.bgColor)
